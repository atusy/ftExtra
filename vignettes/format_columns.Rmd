...
When either dimension is omitted, the optional **magick** package is required
to read the image dimensions. With neither specified, dimensions are calculated
at 72 pixels per inch, following `flextable::as_image()`.
Specify both dimensions (in inches) to avoid automatic sizing.

```{r, eval=knitr::opts_chunk$get("eval") && requireNamespace("magick", quietly = TRUE)}
data.frame(
...
Markdown list items are rendered with text markers and separated by `.sep`
(default: `"\n\n"`). Use `.sep = "\n"` for more compact lists.
Bullet markers are normalized to `•`, and ordered markers to decimal numbers,
starting at the number parsed by Pandoc. Nested lists are indented by two spaces
per level. Inline formatting such as bold, italic, and links is preserved.
List markers are plain text within a single cell paragraph.
...
In Quarto 1.8 or later, `colformat_md()` and `as_paragraph_md()` automatically
use the resolved metadata provided by Quarto during rendering. This includes
bibliography and CSL settings inherited from a book's `_quarto.yml`, directory
metadata, and active profiles. Citation resource paths follow the source
document even when `execute-dir: project` changes the R working directory.
Names not found there remain available to Pandoc's `--resource-path` and
built-in CSL lookup.

With older Quarto versions, or outside rendering, pass the metadata explicitly:
...
Explicit paths are relative to the R working directory. An explicit `metadata`
value takes precedence over automatic detection; use `metadata = list()` to
disable inherited metadata. R Markdown continues to use `rmarkdown::metadata`.
